  ...
  const renderHelpSection = () => (
    <div className="help-section">
      ...
      <div className="help-content">
        ...
        <div className="help-card">
          <h3>✏️ Грамматика</h3>
          <p>Will/going to • Present Continuous vs Simple • would like to • need/want • demonstratives • imperatives • modals • there is/are • count/uncount • possessives</p>
        </div>
        <div className="help-card">
          <h3>🧠 Критическое мышление</h3>
          <p>Реальные офисные сценарии.</p>
        </div>
      </div>
    </div>
  );
  ...
  const renderMainMenu = () => (
    <div className="main-menu">
      ...
      <div className="menu-grid">
        <button onClick={() => setGameState((p) => ({ ...p, currentMode: "vocabulary" }))} className="menu-card vocabulary-card">
          <div className="card-icon">📚</div>
          <h3>Практика Лексики</h3>
          <p>Vocabulary 1/2/3</p>
        </button>

        <button onClick={() => setGameState((p) => ({ ...p, currentMode: "grammar" }))} className="menu-card grammar-card">
          <div className="card-icon">✏️</div>
          <h3>Практика Грамматики</h3>
          <p>Grammar 1/2/3</p>
        </button>

        <button onClick={() => setGameState((p) => ({ ...p, currentMode: "critical_thinking" }))} className="menu-card thinking-card">
          <div className="card-icon">🧠</div>
          <h3>Критическое мышление</h3>
          <p>Офисные ситуации</p>
        </button>

        <button onClick={() => setGameState((p) => ({ ...p, currentMode: "communication" }))} className="menu-card communication-card">
          <div className="card-icon">💬</div>
  ...
  const renderCommunicationMenu = () => (
    <div className="communication-menu">
      <div className="header-section">
        ...
        <button onClick={() => setGameState((p) => ({ ...p, currentMode: "main" }))} className="back-btn">← Назад в главное меню</button>
      </div>

      <div className="communication-grid">
        <button onClick={() => setGameState((p) => ({ ...p, currentMode: "boss" }))} className="communication-card boss-card">
          <div className="card-image"><img src="/boss.png" alt="Босс" /></div>
          <h3>Вызов Босса</h3>
          <p>Профессиональные сценарии</p>
        </button>

        <button onClick={() => setGameState((p) => ({ ...p, currentMode: "client" }))} className="communication-card client-card">
          <div className="card-image"><img src="/client.png" alt="Клиент" /></div>
  ...
  const renderContent = () => {
    ...
    switch (gameState.currentMode) {
      case "vocabulary": {
        ...
        const qs = getVocabQuestions();
        return (
          <QuizGame
            key={`vocab-${selectedVocab}`}  // важный сброс стейта
            questions={qs}
